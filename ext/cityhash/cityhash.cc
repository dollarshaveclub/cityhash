...
{
  return ULL2NUM(CityHash32(StringValueCStr(input), RSTRING_LEN(input)));
}

extern "C" VALUE cityhash_hash64(VALUE mod, VALUE input)
{
  Check_Type(input, T_STRING);
  return ULL2NUM(CityHash64(StringValueCStr(input), RSTRING_LEN(input)));
}

extern "C" VALUE cityhash_hash64_with_seed(VALUE mod, VALUE input, VALUE seed)
{
  Check_Type(input, T_STRING);
  return ULL2NUM(CityHash64WithSeed(StringValueCStr(input), RSTRING_LEN(input), NUM2ULL(seed)));
}

extern "C" VALUE cityhash_hash64_with_seeds(VALUE mod, VALUE input, VALUE seed1, VALUE seed2)
{
  Check_Type(input, T_STRING);
  return ULL2NUM(CityHash64WithSeeds(StringValueCStr(input), RSTRING_LEN(input), NUM2ULL(seed1), NUM2ULL(seed2)));
}

extern "C" VALUE cityhash_hash128(VALUE mod, VALUE input)
{
  Check_Type(input, T_STRING);
  uint128 hash = CityHash128(StringValueCStr(input), RSTRING_LEN(input));
...
extern "C" VALUE cityhash_hash128_with_seed(VALUE mod, VALUE input, VALUE seed_string)
{
  ...
  uint128 hash = CityHash128WithSeed(StringValueCStr(input), RSTRING_LEN(input), seed);
  return rb_str_new((char *)&hash, sizeof(hash));
}
...
{
  uint128 hash = CityHashCrc128(StringValueCStr(input), RSTRING_LEN(input));
  return rb_str_new((char *)&hash, sizeof(hash));
}
...
{
  uint128 seed = *(uint128 *)StringValuePtr(seed_string);
  uint128 hash = CityHashCrc128WithSeed(StringValueCStr(input), RSTRING_LEN(input), seed);
  return rb_str_new((char *)&hash, sizeof(hash));
}
...
{
  uint64 hash[4] = {};
  CityHashCrc256(StringValueCStr(input), RSTRING_LEN(input), hash);
  return rb_str_new((char *)&hash, sizeof(hash));
}
#endif
